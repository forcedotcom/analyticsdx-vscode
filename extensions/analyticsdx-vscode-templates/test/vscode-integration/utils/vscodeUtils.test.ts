...
describe('vscodeUtils', () => {
  ...
  describe('calculateJsonpathAgainst()', () => {
    let dataflowUri: vscode.Uri;
    before(() => {
      dataflowUri = uriFromTestRoot('jsonpathTest', 'SalesAnalyticsDataflow.json');
    });
    ...
    [
      ...
      it(`finds ${count} matches for ${jsonpath}`, async () => {
        const locations = await calculateJsonpathAgainst(jsonpath, dataflowUri);
