...
export class TemplateDirEditing extends Disposable {
  ...
  private _readinessDefinitionPath: string | undefined;
  ...
  public setParsedTemplateInfo(tree: JsonNode | undefined): boolean {
    ...
    path = this.getRelFilePathFromAttr(tree, 'readinessDefinition');
    if (path !== this._readinessDefinitionPath) {
      this._readinessDefinitionPath = path;
      updated = true;
    }
  ...
  get readinessDefinitionPath() {
    return this._readinessDefinitionPath;
  }

  /** Tell if the specified file uri corresponds to our readinessDefinition path. */
  public isReadinessDefinitionFile(file: vscode.Uri): boolean {
    return (
      isUriUnder(this.dir, file) &&
      !!this.readinessDefinitionPath &&
      isValidRelpath(this.readinessDefinitionPath) &&
      isSameUri(vscode.Uri.joinPath(this.dir, this.readinessDefinitionPath), file)
    );
  }
  ...
  public start(): this {
    ...
    this.disposables.push(
      ...
      vscode.languages.registerHoverProvider(relatedFileSelector, new ReadinessVariableHoverProvider(this))
    );
...
export class TemplateEditingManager extends Disposable {
  ...
  public readonly readinessSchemaPath: vscode.Uri;
  ...
  constructor(context: vscode.ExtensionContext, output?: vscode.OutputChannel) {
    ...
    this.readinessSchemaPath = vscode.Uri.file(context.asAbsolutePath(`${schemasPath}/readiness-schema.json`));
  ...
  public getSchemaAssociations(): ISchemaAssociations {
    ...
    this.templateDirs.forEach(editing => {
      if (editing.dir.scheme === 'file') {
        ...
        this.addRelpathSchemaAssociation(
          associations,
          this.readinessSchemaPath,
          editing.dir,
          editing.readinessDefinitionPath
        );
